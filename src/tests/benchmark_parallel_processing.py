...
class BenchmarkResults:
    ...
    def add_result(self, grid_size: int, sequential_time: float, parallel_time: float):
        ...
        self.speedup_factors[grid_size] = sequential_time / parallel_time if parallel_time > 0 else 0
    
    def print_results(self):
        """Print the benchmark results in a formatted table."""
        print("\n" + "=" * 80)
        print(f"{'Grid Size':^15} | {'Sequential (ms)':^20} | {'Parallel (ms)':^20} | {'Speedup Factor':^15}")
        print("-" * 80)
        ...
        for size in sorted(self.grid_sizes):
            ...
            print(f"{size:^15} | {seq_time:^20.2f} | {par_time:^20.2f} | {speedup:^15.2f}")
    ...
    def plot_results(self, title: str, output_file: Optional[str] = None):
        ...
        seq_times = [self.sequential_times[size] * 1000 for size in sorted_sizes]  # Convert to milliseconds
        par_times = [self.parallel_times[size] * 1000 for size in sorted_sizes]    # Convert to milliseconds
...
def benchmark_cellular_automaton(generator: BaseGenerator, grid_sizes: List[int], 
                                iterations: int = 1, runs: int = 3) -> BenchmarkResults:
    """
    ...
    for size in grid_sizes:
        ...
        grid = (grid > 0.7).astype(np.float64)  # Convert to binary grid with ~30% filled
        ...
        for run in range(runs):
            ...
            generator._set_parallel_ca_threshold(size * size * 10)  # Set threshold higher than grid size
        ...
        for run in range(runs):
            ...
            original_threshold = generator._get_parallel_ca_threshold()
            generator._set_parallel_ca_threshold(1)  # Set threshold to 1 to ensure parallel processing
        ...
        logging.info(f"Grid size {size}x{size}: Sequential: {sequential_time:.4f}s, Parallel: {parallel_time:.4f}s, Speedup: {sequential_time/parallel_time:.2f}x")
    
    return results


def benchmark_clustering(generator: BaseGenerator, grid_sizes: List[int], 
                        num_clusters: int = 10, runs: int = 3) -> BenchmarkResults:
    """
    ...
    for size in grid_sizes:
        ...
        # Create a grid with random values
        grid = np.random.random((size, size))
        grid = (grid > 0.5).astype(np.float64)  # Convert to binary grid with ~50% filled
        ...
        for run in range(runs):
            ...
            generator._set_parallel_clustering_threshold(size * size * 10)  # Set threshold higher than grid size
        ...
        for run in range(runs):
            # Force parallel processing by temporarily setting a very low threshold
            original_threshold = generator._get_parallel_clustering_threshold()
            generator._set_parallel_clustering_threshold(1)  # Set threshold to 1 to ensure parallel processing
