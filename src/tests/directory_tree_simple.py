...
DEFAULT_IGNORE = [
    "__pycache__", ".git", ".idea", ".vscode", "*.pyc", "*.pyo", 
    ".DS_Store", ".pytest_cache", "*.egg-info", "dist", "build",
    "venv", "env", ".env", "__init__.py", ".tox", ".coverage"
]
...
def tree(directory, ignore_patterns=None, max_depth=None, prefix="", is_last=False, depth=0):
    ...
    for i, entry in enumerate(entries):
        ...
        if depth == 0:
            ...
            if is_last:
                new_prefix = prefix + "    "  # 4 spaces after last item
            else:
                new_prefix = prefix + "│   "  # vertical line for non-last items
...
def main():
    ...
    if args.output:
        ...
        output_file = os.path.join(os.path.dirname(os.path.abspath(__file__)), f"directory_tree_{dir_name}.md")
