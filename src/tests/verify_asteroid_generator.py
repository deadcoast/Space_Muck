...
class BaseGenerator:
    ...
    def apply_threshold(self, grid, threshold=0.5):
        ...
        # Check if we have a cached result
        cache_key = self._get_cache_key("apply_threshold", threshold=threshold, grid_hash=hash(grid.tobytes()))
        if cache_key in self._cache:
            return self._cache[cache_key]
    ...
    def apply_cellular_automaton(self, grid, iterations=1, rule="conway"):
        ...
        """
        # Check if we have a cached result
        cache_key = self._get_cache_key("apply_cellular_automaton", 
    ...
    def _apply_conway_rule_vectorized(self, grid, neighbors):
        ...
        birth = (neighbors == 3) & (grid == 0)  # Dead cells with exactly 3 neighbors become alive
        survive = ((neighbors == 2) | (neighbors == 3)) & (grid > 0)  # Live cells with 2-3 neighbors survive
    ...
    def _apply_rule_to_cell(self, cell_value, neighbors, rule):
        ...
        if rule == "conway":
            # Conway's Game of Life rules
            if cell_value > 0 and neighbors in [2, 3]:  # Live cell with 2-3 neighbors survives
                return 1.0
    ...
    def apply_clustering(self, grid, min_cluster_size=3):
        ...
        # Check if we have a cached result
        cache_key = self._get_cache_key("apply_clustering", 
...
def test_pattern_generation():
    ...
    print(f"Cached spiral pattern time: {spiral_time2:.4f} seconds (Speed improvement: {spiral_time/spiral_time2:.2f}x if > 1)")
    
    assert np.array_equal(spiral, spiral2), "Cached spiral pattern does not match original"
...
if __name__ == "__main__":
    ...
    print("The refactored AsteroidGenerator is working correctly with the optimized BaseGenerator.")
    print("Performance improvements have been verified through caching and optimized algorithms.")
