        ...
        for backend in backends:
            ...
            for _ in range(repetitions):
                try:
                    ...
                    logging.warning(f"K-means clustering failed with backend {backend}: {e}")
                    kmeans_times.append(float('inf'))

            # Record average time for K-means
            if kmeans_times and not all(t == float('inf') for t in kmeans_times):
                ...
                # Calculate speedup relative to CPU
                if backend != "cpu" and kmeans_cpu_time > 0 and avg_time != float('inf'):
            ...
            for _ in range(repetitions):
                try:
                    ...
                    logging.warning(f"DBSCAN clustering failed with backend {backend}: {e}")
                    dbscan_times.append(float('inf'))

            # Record average time for DBSCAN
            if dbscan_times and not all(t == float('inf') for t in dbscan_times):
                ...
                # Calculate speedup relative to CPU
                if backend != "cpu" and dbscan_cpu_time > 0 and avg_time != float('inf'):
        ...
        for backend in backends:
            ...
            for _ in range(repetitions):
                try:
                    ...
                    logging.warning(f"Terrain generation failed with backend {backend}: {e}")
                    terrain_times.append(float('inf'))

            # Record average time for terrain generation
            if terrain_times and not all(t == float('inf') for t in terrain_times):
                ...
                # Calculate speedup relative to CPU
                if backend != "cpu" and terrain_cpu_time > 0 and avg_time != float('inf'):
                    ...
                    terrain_results["speedup"][backend].append(1.0)  # CPU speedup is 1.0
            ...
            for _ in range(repetitions):
                try:
                    ...
                    if backend == "cpu":
                        ...
                        resources = generate_resource_distribution_gpu(size, size, backend=backend)
                        
                    end_time = time.time()
                    resource_times.append(end_time - start_time)
                except Exception as e:
                    logging.warning(f"Resource distribution failed with backend {backend}: {e}")
            ...
            if resource_times and not all(t == float('inf') for t in resource_times):
                ...
                # Calculate speedup relative to CPU
                if backend != "cpu" and resource_cpu_time > 0 and avg_time != float('inf'):
                    ...
                    resource_results["speedup"][backend].append(1.0)  # CPU speedup is 1.0
...
def benchmark_memory_transfer(
    ...
    for size in data_sizes:
        ...
        for backend in backends:
            ...
            for _ in range(repetitions):
                try:
                    if backend == "cuda" or backend == "cupy":
                        import cupy as cp
            ...
            if h2d_times and not all(t == float('inf') for t in h2d_times):
            ...
            if d2h_times and not all(t == float('inf') for t in d2h_times):
    ...
    for op_name, op_results in results.items():
        if "operation" not in op_results:
            ...
            for sub_op_name, sub_op_results in op_results.items():
                visualize_single_operation(sub_op_results, f"{title} - {op_name} - {sub_op_name}", output_dir)
        else:
...
def visualize_single_operation(
    ...
    if "times" in op_results:
        ...
        plt.grid(True)
        plt.legend()
        plt.xscale('log') if len(x_values) > 1 and max(x_values) / min(x_values) > 100 else None
        plt.yscale('log') if op_results["times"] and any(len(times) > 0 and max(t for t in times if t != float('inf')) / min(t for t in times if t != float('inf') and t > 0) > 100 for times in op_results["times"].values() if times) else None
        plt.tight_layout()
    ...
    if "speedup" in op_results:
        ...
        for backend, speedups in op_results["speedup"].items():
            if backend != "cpu" and speedups and not all(s == 0 for s in speedups):
    ...
    if "bandwidth" in op_results:
        ...
        for backend, bandwidths in op_results["bandwidth"].items():
            if bandwidths and not all(b == 0 for b in bandwidths):
...
def run_all_benchmarks(
    ...
    small_grid_sizes: List[int] = [32, 64, 128, 256, 512, 1024],
    large_data_sizes: List[int] = [1000, 10000, 100000, 1000000],
    repetitions: int = 3
) -> None:
    """
