...
class ProceduralGenerator:
    ...
    def generate_asteroid_field(self, width: int, height: int, density: float = 0.3) -> np.ndarray:
        """
        ...
        try:
            ...
            primary = self._generate_noise_layer(width, height, self.primary_noise, scale=0.02)
            primary = (primary - primary.min()) / (primary.max() - primary.min())
            
            # Generate secondary noise for details
            secondary = self._generate_noise_layer(width, height, self.secondary_noise, scale=0.05)
            ...
            values = np.clip(values, 10, 200).astype(np.int32)  # Clip values to reasonable range
            
            # Assign values to asteroid locations
            grid[asteroid_mask] = values
            
            # Apply cellular automaton to refine the pattern
            grid = self._apply_cellular_automaton(grid, iterations=self.ca_params['iterations'])
    ...
    def generate_rare_minerals(self, grid: np.ndarray, rare_chance: float = 0.1) -> np.ndarray:
        """
        ...
        try:
            ...
            rare_noise = self._generate_noise_layer(width, height, self.rare_noise, scale=0.03)
            rare_noise = (rare_noise - rare_noise.min()) / (rare_noise.max() - rare_noise.min())
            
            # Generate precious noise (even rarer)
            precious_noise = self._generate_noise_layer(width, height, self.rare_noise, scale=0.015)
            precious_noise = (precious_noise - precious_noise.min()) / (precious_noise.max() - precious_noise.min())
            
            # Generate anomaly noise (extremely rare)
            anomaly_noise = self._generate_noise_layer(width, height, self.anomaly_noise, scale=0.01)
            anomaly_noise = (anomaly_noise - anomaly_noise.min()) / (anomaly_noise.max() - anomaly_noise.min())
            ...
            for mineral_type in [1, 2, 3]:
                ...
                if np.sum(type_mask) > 0:
                    ...
                    growth_prob = 0.3 if mineral_type == 1 else 0.2 if mineral_type == 2 else 0.1
    ...
    def generate_symbiote_rules(self, race_id: int, hunger: float, genome: Dict[str, float]) -> Tuple[Set[int], Set[int]]:
        """
        ...
        if adaptability > 0.7:
            # High adaptability - more birth conditions
            if random.random() < adaptability:
    ...
    def place_symbiote_race(self, width: int, height: int, race_id: int, initial_density: float = 0.005, pattern: str = 'random') -> np.ndarray:
        """
    ...
    def generate_resource_hotspots(self, grid: np.ndarray, num_hotspots: int = 3) -> np.ndarray:
        """
    ...
    def simulate_symbiote_evolution(
        ...
        with LogContext("simulate_symbiote_evolution"):
            ...
            if food_value > 0:
                ...
                efficiency = 0.05 * metabolism_factor  # Base of 5% population increase per food unit
                
                # Growth is also affected by current population (logistic growth)
                carrying_capacity = self.evolution_params['carrying_capacity'] * (1 + food_value / 100)
                ...
                satiation = min(1.0, food_value / (population * 0.1))  # How satisfied they are
            ...
            for mineral_type, amount in minerals.items():
                ...
                for _ in range(amount):
                    ...
                    if random.random() < self.evolution_params['mutation_chance'].get(mineral_type, 0):
                        # Determine which attribute mutates
                        attrs = list(genome.keys())
                        attr = random.choice(attrs)
                        
                        # Calculate mutation magnitude
                        base_magnitude = self.evolution_params['mutation_magnitude'].get(mineral_type, 1.05)
                        ...
                        if mineral_type == 'anomaly':
                            ...
                            if effect == "population_surge":
                                ...
                                new_population = min(int(new_population * 1.3), carrying_capacity)
                                ...
                                mutations.append({
                                    ...
                                    "magnitude": random.choice([0.7, 1.4]),  # Either much higher or lower
                                    "mineral_type": mineral_type
                                })
            
            return new_population, new_aggression, mutations

    def _generate_noise_layer(self, width: int, height: int, noise_fn: PerlinNoise, scale: float = 0.02) -> np.ndarray:
        """
    ...
    def _apply_cellular_automaton(self, grid: np.ndarray, iterations: int = 3) -> np.ndarray:
        """
        ...
        for _ in range(iterations):
            ...
            neighbors = ndimage.convolve(binary_grid, kernel, mode='wrap' if self.ca_params['wrap'] else 'constant')
            
            # Apply Conway's Game of Life rules
            # Birth rule: empty cell with exactly 3 neighbors becomes filled
            birth_mask = (binary_grid == 0) & np.isin(neighbors, list(self.ca_params['birth_set']))
            # Survival rule: filled cell with 2 or 3 neighbors stays filled
            survival_mask = (binary_grid == 1) & np.isin(neighbors, list(self.ca_params['survival_set']))
    ...
    def _create_value_clusters(self, grid: np.ndarray) -> np.ndarray:
        ...
        n_clusters = min(n_clusters, len(asteroid_coords) // 10)  # Ensure reasonable cluster size
        ...
        try:
            ...
            kmeans = KMeans(n_clusters=n_clusters, random_state=self.seed).fit(asteroid_coords)
            ...
            for i, (y, x) in enumerate(asteroid_coords):
                ...
                max_dist = np.sqrt(grid.shape[0]**2 + grid.shape[1]**2) * 0.1  # 10% of diagonal
                distance_factor = max(0, 1 - distance / max_dist)
                
                # Apply value boost based on distance from center
                if distance_factor > 0:
                    multiplier = 1 + (cluster_multipliers[cluster_id] - 1) * distance_factor
    ...
    def _create_void_areas(self, grid: np.ndarray, count: int = 3, max_size: int = 50) -> np.ndarray:
        """
    ...
    def _apply_edge_degradation(self, grid: np.ndarray, decay_factor: float = 0.1) -> np.ndarray:
        """
    ...
    def _apply_cellular_automaton(self, grid: np.ndarray, iterations: int = 3) -> np.ndarray:
        ...
        for _ in range(iterations):
            ...
            for y in range(grid.shape[0]):
                for x in range(grid.shape[1]):
                    neighbors = grid[max(0, y - 1):min(grid.shape[0], y + 2), max(0, x - 1):min(grid.shape[1], x + 2)]
...
class ProceduralGenerator:
    ...
    def _apply_random_noise(self, grid: np.ndarray, noise_factor: float = 0.1) -> np.ndarray:
        """
        ...
        for _ in range(iterations):
            ...
            for y in range(height):
                for x in range(width):
                    ...
                    if binary_grid[y, x] == 1 and neighbors_count in self.ca_params['survival']:
                        new_binary[y, x] = 1
                    elif binary_grid[y, x] == 0 and neighbors_count in self.ca_params['birth']:
                        new_binary[y, x] = 1
    ...
    def _apply_perlin_noise(self, grid: np.ndarray, scale: float = 10.0) -> np.ndarray:
        ...
        return (noise([x / scale for x in range(grid.shape[1])] for y in range(grid.shape[0])) * 255).astype(np.int8)
    ...
    def _apply_morphological_opening(self, grid: np.ndarray, iterations: int = 1) -> np.ndarray:
        """
    ...
    def _apply_morphological_closing(self, grid: np.ndarray, iterations: int = 1) -> np.ndarray:
        """
    ...
    def _apply_morphological_erosion(self, grid: np.ndarray, iterations: int = 1) -> np.ndarray:
        """
    ...
    def _apply_morphological_dilation(self, grid: np.ndarray, iterations: int = 1) -> np.ndarray:
        """
...
from src.config import *
from src.utils.logging_setup import log_exception, LogContext, log_performance_start, log_performance_end
...
class AsteroidGenerator:
    ...
    def generate_field(self, pattern_weights: Optional[List[float]] = None) -> Tuple[np.ndarray, Dict[str, Any]]:
        ...
        if pattern_weights and len(pattern_weights) == len(self.patterns):
            ...
            grid = noise_grid * (1 - self.pattern_strength) + pattern_grid * self.pattern_strength
        else:
            grid = noise_grid
            ...
            for y in range(max(0, cy - radius), min(self.height, cy + radius + 1)):
                for x in range(max(0, cx - radius), min(self.width, cx + radius + 1)):
                    if asteroid_grid[y, x] > 0:
                        ...
                        if dist <= radius and random.random() < (1 - dist/radius)**2:
                            rare_grid[y, x] = 1
    ...
    def generate_anomalies(self, asteroid_grid: np.ndarray) -> np.ndarray:
        ...
        if num_features > 0:
            ...
            for _ in range(min(num_anomalies, num_features)):
                ...
                # Choose a random point in the component
                if points:
                    y, x = random.choice(points)
                    anomaly_grid[y, x] = random.randint(1, 3)  # Different types of anomalies
    ...
    def _apply_cellular_automaton(self, grid: np.ndarray, iterations: int = 2) -> np.ndarray:
...
class SymbioteEvolutionGenerator:
    ...
    def generate_race_parameters(self, race_type: Optional[str] = None) -> Dict[str, Any]:
        """
    ...
    def generate_initial_population(
        ...
        if race_trait == "adaptive":
                    ...
                    if has_asteroids:
                        ...
                        for dy in range(-radius, radius + 1):
                            for dx in range(-radius, radius + 1):
                                ...
                                if (0 <= nx < self.width and 0 <= ny < self.height):
                                    dist = math.sqrt(dx*dx + dy*dy)
                                    if dist <= radius and random.random() < 0.7 - (dist / radius) * 0.5:
            ...
            if len(asteroid_points) > 10:
                ...
                kmeans = KMeans(n_clusters=num_clusters, random_state=42).fit(points_array)
                ...
                for center in kmeans.cluster_centers_:
                    ...
                    for dy in range(-radius, radius + 1):
                        for dx in range(-radius, radius + 1):
                            ...
                            if (0 <= nx < self.width and 0 <= ny < self.height):
                                dist = math.sqrt(dx*dx + dy*dy)
                                if dist <= radius and random.random() < 0.5 - (dist / radius) * 0.3:
                                    population_grid[ny, nx] = 1
            else:
