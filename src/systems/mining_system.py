...
class MiningSystem:
    ...
    def add_miner(self, miner: Union[MinerEntity, EnhancedMinerEntity]) -> None:
        ...
        if miner.race_id in self.miners:
            self.logger.warning(
                f"Replacing existing miner with race_id {miner.race_id}"
            )
    ...
    def remove_miner(self, race_id: int) -> None:
        ...
        if race_id in self.miners:
            ...
            self.logger.warning(
                f"Attempted to remove non-existent miner with race_id {race_id}"
            )
    ...
    def _process_resource_extraction(
        self, asteroid_field: Any, cell_claims: Dict[Tuple[int, int], List[int]]
    ) -> None:
        """
        ...
        for cell, claiming_races in cell_claims.items():
            ...
            if len(claiming_races) > 1:
                ...
                self._allocate_resources_to_single_race(
                    cell_resources, claiming_races[0]
                )

    def _allocate_resources_to_single_race(
        self, cell_resources: Dict[str, int], race_id: int
    ) -> None:
        """
    ...
    def _distribute_contested_resources(
        self, resources: Dict[str, int], claiming_races: List[int]
    ) -> None:
        """
        ...
        for race_id in claiming_races:
            ...
            strength = (
                miner.population * miner.mining_efficiency * (1 + miner.aggression)
            )
        ...
        for race_id, miner in self.miners.items():
            ...
            if hasattr(miner, "get_territory"):
                try:
                    ...
                    self.logger.error(
                        f"Error getting territory for race {race_id}: {e}"
                    )
