        ...
        with LogContext("GameLoop.update"):
            ...
            for update_func in self.update_functions:
                try:
                    ...
                    logging.error(f"Error in update function {update_func.__name__}: {e}")
        ...
        with LogContext("GameLoop.render"):
            for render_func in self.render_functions:
                try:
                    ...
                    logging.error(f"Error in render function {render_func.__name__}: {e}")
    
    def run(self, clock_func: Callable[[], float], frame_limit: Optional[int] = None) -> None:
        """
