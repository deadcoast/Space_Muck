"""
src/examples/advanced_mining_example.py
...
def run_advanced_mining_example():
    ...
    asteroid_field = AsteroidField(width=FIELD_WIDTH, height=FIELD_HEIGHT, density=0.3)
...
def render_stage_distribution(
    surface: pygame.Surface, mining_system: MiningSystem, x: int, y: int
) -> None:
    """
        ...
        for stage in ["elder", "specialized", "worker", "juvenile"]:
            ...
            if stage_pop > 0:
                ...
                segment_rect = pygame.Rect(
                    bar_x, current_y - segment_height, bar_width, segment_height
                )
...
def render_population_graph(
    ...
    for race_id in range(1, 4):
        ...
        for i in range(20):
            ...
            t = i / 19
            history_pop = int(
                current_pop * t + 50 * (1 - t) * (1 + 0.5 * np.sin(i * 0.4))
            )
