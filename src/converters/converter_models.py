...
class Recipe:
    ...
    def is_compatible_with(self, converter: "Converter") -> bool:
        ...
        return (
            converter.type == self.required_converter_type
            and converter.tier.value >= self.min_converter_tier.value
        )
...
class Converter:
    ...
    def start_process(
        ...
        for resource_type, amount in recipe.inputs.items():
            if (
                resource_type not in input_resources
                or input_resources[resource_type] < amount
            ):
                return None
    ...
    def update(
        ...
        for process in self.active_processes:
            ...
            efficiency = self.get_overall_efficiency(
                process.quality_modifier, technology_modifier, environmental_modifier
            )

            # Check if we have enough energy
            energy_needed = (
                process.recipe.get_energy_with_efficiency(efficiency) * delta_time
            )
            if self.current_energy >= energy_needed:
                ...
                # Update progress
                process.progress += (
                    delta_time / process.recipe.get_time_with_efficiency(efficiency)
                )
...
class ConversionProcess:
    ...
    def __init__(
        self, id: str, recipe: Recipe, converter_id: str, quality_modifier: float = 1.0
    ):
        """
...
class ChainStep:
    ...
    def __init__(self, converter_id: str, recipe_id: str, step_order: int):
        """
...
class ProductionChain:
    ...
    def __init__(
        self, id: str, name: str, steps: List[ChainStep], description: str = ""
    ):
        """
