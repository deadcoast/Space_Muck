"""
src/generators/asteroid_field.py
...
if TYPE_CHECKING:
    from algorithms.cellular_automaton import (  # type: ignore
        apply_life_rules,
        count_neighbors,
        diffuse_energy,
    )
...
if CELLULAR_AUTOMATON_AVAILABLE:
    try:
        from algorithms.cellular_automaton import (
            apply_life_rules,
            count_neighbors,
            diffuse_energy,
        )
        ...
        logging.warning(
            "Optimized cellular automaton algorithms not available. Using internal implementation."
        )
...
if EXTENDED_CA_AVAILABLE:
    try:
        ...
        logging.warning(
            "Extended cellular automaton features not available. Using basic implementation."
        )
...
class AsteroidField:
    ...
    def _apply_cellular_automaton_scipy(
        ...
        if self.use_custom_neighborhood and EXTENDED_CA_AVAILABLE:
            try:
                ...
                logging.warning(
                    f"Error using custom neighborhood: {str(e)}. Falling back to standard convolution."
                )
    ...
    def _get_neighborhood_offsets(pattern: str) -> List[Tuple[int, int]]:
        ...
        if pattern == "directional":
            ...
            logging.warning(
                f"Unknown neighborhood pattern: {pattern}. Using Moore neighborhood."
            )
            return [
                (-1, -1),
                (-1, 0),
    ...
    def _calculate_custom_neighbors_manually(self, grid):
        ...
        for y in range(self.height):
            for x in range(self.width):
                neighbor_counts[y, x] = self._count_custom_neighbors_at_position(
                    grid, x, y
                )
    ...
    def _count_neighbors_manual(self, grid, x, y):
        ...
        offsets = (
            self.custom_neighborhood_offsets
            if self.use_custom_neighborhood
    ...
    def _apply_automaton_with_energy(
        ...
        if CELLULAR_AUTOMATON_AVAILABLE:
            try:
                ...
                logging.warning(
                    f"Falling back to internal cellular automaton implementation: {str(e)}"
                )
    ...
    def _low_density_mask(self, binary_grid):
        ...
        self.energy_grid = diffuse_energy(
            boosted_energy, decay_rate=self.energy_decay, spread_rate=self.energy_spread
        )
