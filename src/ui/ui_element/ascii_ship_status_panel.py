...
class ASCIIShipStatusPanel:
    ...
    def update_hull_shield(
        ...
        try:
            ...
            self.hull_integrity = (
                int((current_hull / max_hull) * 100) if max_hull > 0 else 0
            )

            self.current_shield = current_shield
            self.max_shield = max_shield
            self.shield_integrity = (
                int((current_shield / max_shield) * 100) if max_shield > 0 else 0
            )
    ...
    def draw(self, surface: pygame.Surface, font: pygame.font.Font) -> pygame.Rect:
        ...
        try:
            ...
            surface.blit(
                font.render(ship_info, True, COLOR_HIGHLIGHT), (content_x, content_y)
            )
            ...
            self._draw_status_bar(
                ...
                "Hull",
                self.hull_integrity,
                self._get_percentage_color(self.hull_integrity),
            )
            ...
            self._draw_status_bar(
                ...
                "Shield",
                self.shield_integrity,
                self._get_percentage_color(self.shield_integrity),
            )
            content_y += font.get_height() + margin // 2
            ...
            surface.blit(
                font.render(power_header, True, COLOR_HIGHLIGHT), (content_x, content_y)
            )
            content_y += font.get_height() + margin // 2

            # Power bar
            power_percentage = (
                int((self.current_power / self.max_power) * 100)
                if self.max_power > 0
                else 0
            )
            self._draw_status_bar(
                ...
                "Power",
                power_percentage,
                self._get_percentage_color(power_percentage),
            )
            content_y += font.get_height() + margin // 2
            ...
            balance_color = (
                self.status_colors["good"]
                if power_balance >= 0
                else self.status_colors["warning"]
            )

            power_text = (
                f"Generation: {self.power_generation}  Usage: {self.power_usage}  "
            )
            power_text += f"Balance: {power_balance:+.1f}"

            surface.blit(
                font.render(power_text, True, balance_color), (content_x, content_y)
            )
            content_y += font.get_height() + margin
            ...
            # Draw system headers
            surface.blit(
                font.render("System", True, COLOR_TEXT), (content_x, content_y)
            )
            ...
            for system_name, system_data in self.systems.items():
                ...
                surface.blit(
                    font.render(display_name, True, COLOR_TEXT), (content_x, content_y)
                )
            ...
            if content_y + font.get_height() * 4 < self.rect.bottom - margin:
                ...
                surface.blit(
                    font.render(attack_text, True, COLOR_TEXT), (content_x, content_y)
                )
                ...
                surface.blit(
                    font.render(crit_text, True, COLOR_TEXT), (content_x, content_y)
                )
