...
from typing import Tuple, List, Dict, Optional, Any, Callable, TypeVar
import contextlib
import pygame
from src.ui.ui_base.ascii_base import UIStyle
from src.ui.draw_utils import draw_text
from src.ui.ui_event import UIEventType, UIEventData
...
def handle_mouse_event(
    ...
    try:
        ...
        is_inside = (
            ...
            and self.y * char_height <= pixel_y < (self.y + self.height) * char_height
        )
...
def draw(
    ...
    try:
        ...
        # Draw borders
        self._draw_borders(box_surf, font, adjusted_base_color, char_width, char_height)
...
def _draw_quantum_animation(
    ...
    try:
        ...
        for y, x in itertools.product(range(self.height), range(self.width)):
            ...
            wave = math.sin(x / complexity_factor + y / complexity_factor + time_factor)
            ...
            idx = min(int((combined_wave + 1) * 0.5 * (len(chars) - 1)), len(chars) - 1)
            char = chars[idx]
...
def _get_animation_char_and_color(
    ...
    if self.style == UIStyle.QUANTUM:
        chars = ["·", "∙", "•", "◦", "○", "◌", "◍", "◎", "●"]
        idx = min(int(self.animation["progress"] * (len(chars) - 1)), len(chars) - 1)
...
def _draw_borders(
    ...
    # Top border
    top_border = (
        self.borders["tl"] + self.borders["h"] * (self.width - 2) + self.borders["tr"]
    )
    draw_text(surface, top_border, 0, 0, size=font.get_height(), color=base_color)

    # Bottom border
    bottom_border = (
        self.borders["bl"] + self.borders["h"] * (self.width - 2) + self.borders["br"]
    )
    draw_text(
        surface,
        bottom_border,
...
try:
    ...
    try:
        if self.style == UIStyle.QUANTUM:
            ...
            focus_color = self._enhance_color(base_color, red_boost=50, green_boost=50)
...
def _get_hover_color(self, base_color: Tuple[int, int, int]) -> Tuple[int, int, int]:
