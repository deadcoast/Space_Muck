...
class NotificationManager:
    ...
    def __init__(self) -> None:
        ...
        self.max_visible_notifications = 10  # Maximum number of notifications shown at once
        ...
        self.compact_notification_count = 3  # Number of notifications to show when compact
    ...
    def update(self) -> None:
        ...
        for notification in self.notifications:
            if not notification["read"] and current_time - notification["timestamp"] > self.highlight_time:
    ...
    def draw(self, surface: pygame.Surface) -> None:
        ...
        if not self.show_full_panel and self.fade_timer <= 0:
            ...
            visible_count = min(self.compact_notification_count, len(filtered_notifications))
            ...
            if visible_count > 0:
                ...
                for i in range(visible_count):
                    notification = filtered_notifications[i]
                    ...
                    if notification_age < self.highlight_time:
                        ...
                        pygame.draw.rect(
                            surface,
                            (60, 60, 80, alpha),
                            (self.panel_x + 5, self.panel_y + 5 + i * self.notification_height,
                             self.panel_width - 10, self.notification_height - 2),
                            border_radius=3
                        )
                    ...
                    draw_text(
                        ...
                        text,
                        self.panel_x + 10,
                        self.panel_y + 5 + i * self.notification_height + 4,
                        14,
                        color,
                        alpha=alpha
                    )
                ...
                if len(filtered_notifications) > visible_count:
                    draw_text(
                        surface,
                        f"+ {len(filtered_notifications) - visible_count} more (Press N)",
        ...
        # Draw the full notification panel
        if self.show_full_panel or self.fade_timer > 0:
            ...
            if visible_count > 0:
                ...
                end_idx = min(start_idx + self.max_visible_notifications, len(filtered_notifications))
                ...
                for i, notification in enumerate(visible_notifications):
                    ...
                    pygame.draw.rect(
                        surface,
                        bg_color,
                        (self.panel_x + 5, notification_y,
                         self.panel_width - 20, self.notification_height - 2),
                        border_radius=3
                    )
    ...
    def handle_event(self, event: pygame.event.Event) -> bool:
        ...
        if event.type == pygame.KEYDOWN:
            ...
            if event.key == pygame.K_n:
                ...
                self.fade_timer = 180 if not self.show_full_panel else 0
                return True
                
            # Scroll notifications
            if self.show_full_panel:
                filtered_notifications = self.get_filtered_notifications()
                max_scroll = max(0, len(filtered_notifications) - self.max_visible_notifications)
                ...
                # Page up/down
                    ...
                    self.scroll_offset = max(0, self.scroll_offset - self.max_visible_notifications)
                    return True
                    
                elif event.key == pygame.K_PAGEDOWN:
                    self.scroll_offset = min(max_scroll, self.scroll_offset + self.max_visible_notifications)
                    return True
            ...
            # Check if click was in the notification panel
            if self.show_full_panel:
                # Check for category filter clicks
                if self.panel_x <= event.pos[0] <= self.panel_x + self.panel_width:
                    # Category filters
    ...
    def notify_event(self, type_str: str, message: str, importance: int = 1) -> None:
        ...
        if type_str == "discovery":
            self.add(f"DISCOVERY: {message}", color=(0, 255, 200), category="event", importance=2)
        elif type_str == "warning":
            self.add(f"WARNING: {message}", color=(255, 100, 100), category="event", importance=3)
        elif type_str == "achievement":
            self.add(f"ACHIEVEMENT: {message}", color=(255, 215, 0), category="event", importance=2)
        elif type_str == "race":
            self.add(f"RACE EVENT: {message}", color=(200, 100, 255), category="race", importance=2)
        elif type_str == "mining":
            self.add(message, color=(200, 200, 200), category="mining", importance=1)
        elif type_str == "upgrade":
            self.add(f"UPGRADE: {message}", color=(100, 255, 100), category="upgrade", importance=2)
        else:
