...
from src.ui.draw_utils import draw_text, draw_panel, draw_button, draw_tooltip
...
class Shop:
    ...
    def __init__(self) -> None:
        ...
        self.position_expanded: Tuple[int, int, int, int] = (
            WINDOW_WIDTH - 350, 50, 300, WINDOW_HEIGHT - 100
        )
        self.position_collapsed: Tuple[int, int, int, int] = (
            WINDOW_WIDTH - 120, 50, 100, 200
        )
    ...
    def _initialize_options(self) -> None:
        ...
        self.options.extend([
            ...
            },
            {
                ...
                "action": lambda player, field: setattr(
                    field, "rare_bonus_multiplier", field.rare_bonus_multiplier + 0.5
                ),
                "category": "field",
                "icon": "🔮",
                "max_level": 5,
        ...
        self.options.extend([
            {
                ...
                "action": lambda player, field, notifier: self.discover_race(field, notifier),
                "category": "race",
                "icon": "👽",
                "max_level": 3,
                "current_level": 0,
            ...
            },
            {
                ...
                "action": lambda player, field, notifier: self._feed_races(player, field, 50, notifier),
                "category": "race",
                "icon": "🍽️",
            ...
            },
            {
                ...
                "action": lambda player, field, notifier: self._feed_races(player, field, 150, notifier),
                "category": "race",
                "icon": "🍲",
                "max_level": 0,  # No level limit
                "current_level": 0,
            ...
            },
            {
                ...
                "action": lambda player, field, notifier: self._feed_races(player, field, 500, notifier),
                "category": "race",
                "icon": "🍱",
                "max_level": 0,  # No level limit
                "current_level": 0,
        ...
        self.options.extend([
            {
                ...
                "action": lambda player, field: setattr(player, "auto_upgrade", True),
                "category": "special",
                "icon": "🔄",
                "max_level": 1,
                "current_level": 0,
        ...
        self.hidden_options = [
            {
                ...
                "action": lambda player, field, notifier: self._unlock_anomaly_detection(player, field, notifier),
                "category": "special",
                "icon": "📡",
                "max_level": 1,
                "current_level": 0,
            ...
            },
            {
                ...
                "action": lambda player, field, notifier: self._unlock_symbiote_communication(player, field, notifier),
                "category": "race",
                "icon": "🔊",
                "max_level": 1,
                "current_level": 0,
                "unlock_condition": lambda player, field: len(field.races) >= 2 and any(race.evolution_stage >= 2 for race in field.races),
                "unlock_message": "Symbiote Communication technology now available!",
            },
        ]

    def check_unlockable_options(self, player: Player, field: AsteroidField, notifier: NotificationManager) -> None:
        """
        ...
        for option in list(self.hidden_options):
            if "unlock_condition" in option and option["unlock_condition"](player, field):
    ...
    def get_filtered_options(self, player: Player) -> List[Dict[str, Any]]:
        ...
        filtered_options.sort(
            key=lambda x: (
                player.currency < self._get_option_cost(x, player),  # Affordable options first
                x["current_level"] >= x["max_level"] if x["max_level"] > 0 else False,  # Available levels first
    ...
    def _feed_races(self, player: Player, field: AsteroidField, amount: int, notifier: NotificationManager) -> None:
        """
        ...
        results = player.feed_symbiotes(field, amount)
        notifier.add(
            f"Fed {amount} minerals to symbiotic races.",
            category="race"
        )
        
        # Check if feeding triggered any evolution
        for race in field.races:
            if race.fed_this_turn and race.hunger < 0.3 and race.evolution_points >= race.evolution_threshold:
                metrics = race.evolve()
                notifier.add(
    ...
    def discover_race(self, field: AsteroidField, notifier: NotificationManager) -> bool:
        """
        ...
        if len(field.races) >= 3:
            notifier.add(
                "No more undiscovered races in this sector.",
                category="race"
            )
            return False
    ...
    def purchase_upgrade(self, option: Dict[str, Any], player: Player, field: AsteroidField, notifier: NotificationManager) -> bool:
        """
        ...
        if option["max_level"] > 0 and option["current_level"] >= option["max_level"]:
            notifier.add(
                f"Maximum level reached for {option['name']}.",
                category="upgrade"
            )
            return False
        ...
        if "requires" in option:
            if callable(option["requires"]):
                if not option["requires"](player, field):
                    notifier.add(
    ...
    def _unlock_anomaly_detection(self, player: Player, field: AsteroidField, notifier: NotificationManager) -> None:
    ...
    def _unlock_symbiote_communication(self, player: Player, field: AsteroidField, notifier: NotificationManager) -> None:
    ...
    def update_animation(self, delta_time: float) -> None:
        ...
        if self.animation_progress < 1.0:
            ...
            animation_speed = 3.0
            self.animation_progress = min(1.0, self.animation_progress + delta_time * animation_speed)
    ...
    def get_current_panel_rect(self) -> pygame.Rect:
        ...
        if self.animation_progress >= 1.0:
            if self.expanded:
                return pygame.Rect(self.position_expanded)
            else:
                return pygame.Rect(self.position_collapsed)
        else:
    ...
    def draw(self, surface: pygame.Surface, player: Player) -> None:
        ...
        toggle_button_rect = pygame.Rect(
            panel_rect.x + panel_rect.width - 30,
            panel_rect.y + 5,
            20,
            20
        )
        ...
        for i in range(visible_count):
            ...
            bg_color = (60, 60, 80) if is_selected else \
                      (50, 50, 70) if is_hovered else \
                      (40, 40, 55)
            ...
            max_level_reached = option["max_level"] > 0 and option["current_level"] >= option["max_level"]
            ...
            # Draw item border
            border_color = (100, 200, 100) if can_afford and not max_level_reached and requirements_met else \
                          (200, 100, 100) if max_level_reached or not requirements_met else \
                          (150, 150, 150)
            ...
            # Draw item name
            name_color = COLOR_TEXT if can_afford and not max_level_reached and requirements_met else \
                         (200, 100, 100) if max_level_reached or not requirements_met else \
                         (150, 150, 150)
            draw_text(
                surface,
            ...
            description_color = COLOR_TEXT if can_afford and not max_level_reached and requirements_met else \
                                    (200, 100, 100) if max_level_reached or not requirements_met else \
                                    (150, 150, 150)
            draw_text(
                surface,
            ...
            # Draw item level
            level_color = COLOR_TEXT if can_afford and not max_level_reached and requirements_met else \
                            (200, 100, 100) if max_level_reached or not requirements_met else \
                            (150, 150, 150)
            draw_text(
                surface,
        ...
        if self.expanded:
            draw_button(surface, "Collapse", button_rect, COLOR_BUTTON, COLOR_BUTTON_HOVER)
        else:
            draw_button(surface, "Expand", button_rect, COLOR_BUTTON, COLOR_BUTTON_HOVER)
        
        # Draw tooltip
        if self.selected_item is not None:
            option = self.options[self.selected_item]
            draw_tooltip(surface, option["name"], option["description"], option["current_level"])
        ...
        for category in self.categories:
            ...
            if category == self.current_category:
                draw_button(surface, category.capitalize(), button_rect, COLOR_BUTTON, COLOR_BUTTON_HOVER)
            else:
                draw_button(surface, category.capitalize(), button_rect, COLOR_BUTTON, COLOR_BUTTON_HOVER)
        ...
        help_rect = pygame.Rect(
            panel_rect.x + panel_rect.width - 60,
            panel_rect.y + 10,
            25,
            25
        )
...
class Shop:
    ...
    def purchase_upgrade(self, option: Dict[str, Any], player: Player, field: AsteroidField, notifier: NotificationManager) -> bool:
        ...
        # Check requirements
        if "requires" in option and (callable(option["requires"]) and not option["requires"](player, field)):
            notifier.add(
                f"Requirements not met for {option['name']}.",
                category="upgrade"
            )
    ...
    def draw(self, surface: pygame.Surface, player: Player) -> None:
        ...
        for i in range(visible_count):
            ...
            # Draw item cost
            cost_color = COLOR_TEXT if can_afford and not max_level_reached and requirements_met else \
                            (200, 100, 100) if max_level_reached or not requirements_met else \
                            (150, 150, 150)
            draw_text(
                surface,
                f"{cost} {self.currency_name}",
                item_rect.x + item_rect.width - 10,
                item_rect.y + 2,
                20,
                cost_color,
                align="right"
            ...
            level_color = COLOR_TEXT if can_afford and not max_level_reached and requirements_met else \
                            (200, 100, 100) if max_level_reached or not requirements_met else \
                            (150, 150, 150)
            level_text = f"Level {option['current_level']}/{option['max_level']}" if option["max_level"] > 0 else \
                            f"Level {option['current_level']}"
            draw_text(
                surface,
                level_text,
            ...
            separator_rect = pygame.Rect(
                item_rect.x + 10,
                item_rect.y + self.item_height - 5,
                item_rect.width - 20,
                1
            )
            ...
            # Draw tooltip
            if self.show_tooltip and is_hovered:
                self.draw_tooltip(surface, option, item_rect, player)
            ...
            self.selected_item = item_index if item_rect.collidepoint(mouse_pos) else -1
            # Check if item is clicked
            if item_rect.collidepoint(mouse_pos) and mouse_click[0]:
                if can_afford and not max_level_reached and requirements_met:
                    option["upgrade_function"](player)
