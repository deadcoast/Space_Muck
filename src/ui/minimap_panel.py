...
class ASCIIMinimapPanel:
    ...
    def _generate_empty_map(self) -> List[List[str]]:
        ...
        return [["empty" for _ in range(self.map_width)] for _ in range(self.map_height)]
    
    def update_map(self, map_data: List[List[str]], player_pos: Tuple[int, int], 
                  points_of_interest: List[Dict[str, Any]]) -> None:
        """
        Update the map data.
    ...
    def update(self, player_position=None, grid=None, entity_grid=None, field_offset_x=None, field_offset_y=None, zoom_level=None) -> None:
        ...
        self.animation_state["pulse"] = (
            self.animation_state["pulse"] + dt * 2.0
        ) % (2.0 * math.pi)
    ...
    def update_animations(self, delta_time: float) -> None:
        ...
        if self.animation_state["active"]:
            self.animation_state["progress"] = min(
                1.0, self.animation_state["progress"] + delta_time / self.animation_state["duration"]
            )
            if self.animation_state["progress"] >= 1.0:
                self.animation_state["active"] = False
    ...
    def generate_random_map(self, density: float = 0.3) -> None:
        ...
        weights = [1.0 - density, density * 0.7, density * 0.1, density * 0.1, density * 0.1]
