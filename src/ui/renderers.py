...
from src.ui.draw_utils import draw_text, draw_panel, draw_minimap
...
class AsteroidFieldRenderer:
    ...
    def __init__(self) -> None:
        ...
        for i in range(101):
            ...
            if normalized <= 0.5:
                ...
                t = normalized / 0.5
                color = self._interpolate_color(self.asteroid_gradient[0], self.asteroid_gradient[1], t)
            else:
                # Mix second and third colors
                t = (normalized - 0.5) / 0.5
                color = self._interpolate_color(self.asteroid_gradient[1], self.asteroid_gradient[2], t)
    ...
    def _init_cell_surfaces(self) -> None:
        ...
        for i in range(0, 101, 5):  # Create every 5th value to save memory
            ...
            pygame.draw.circle(surface, color, (base_size//2, base_size//2), base_size//2 - 1)
            ...
            pygame.draw.circle(surface, highlight, (base_size//3, base_size//3), base_size//6)
            pygame.draw.circle(surface, shadow, (2*base_size//3, 2*base_size//3), base_size//5)
            
            self.cell_surfaces[i] = surface
            
        # Create rare overlay
        self.rare_overlay = pygame.Surface((base_size, base_size), pygame.SRCALPHA)
        pygame.draw.circle(self.rare_overlay, (*self.rare_color, 180), (base_size//2, base_size//2), base_size//2 - 2, 2)
        ...
        for y in range(min_y, min(max_y + 1, grid.shape[0])):
            for x in range(min_x, min(max_x + 1, grid.shape[1])):
                ...
                if value > 0:
                    ...
                    if cell_size >= 8:
                        # Use pre-rendered surfaces for larger cell sizes
                        cell_surface = self.get_cell_surface(value, cell_size)
                        if cell_surface:
                            surface.blit(cell_surface, (screen_x, screen_y))
                            
                            # Add rare overlay if needed
                            if is_rare:
                                rare_scaled = pygame.transform.scale(self.rare_overlay, (cell_size, cell_size))
...
class PlayerRenderer:
    ...
    def render_player(
        ...
        if moving:
            ...
            # Get current exhaust frame
            frame_idx = int(self.animation_counter)
            exhaust_surf = self.exhaust_animation[frame_idx]
            
            # Scale exhaust to match ship
            scaled_exhaust = pygame.transform.scale(
                exhaust_surf, 
                (ship_size, ship_size//2)
            )
        ...
        for i in range(1, len(ship_positions)):
            ...
            if min_x <= ship_x <= max_x and min_y <= ship_y <= max_y:
                ...
                if health < 100:
                    ...
                    pygame.draw.rect(
                        surface,
                        (0, 255, 0) if health > 50 else (255, 255, 0) if health > 25 else (255, 0, 0),
                        (screen_x, screen_y - bar_height - 1, int(bar_width * health / 100), bar_height)
                    )
...
class EffectsRenderer:
    ...
    def add_effect(self, effect_type: str, x: int, y: int, **kwargs) -> None:
        ...
        if effect_type == "explosion":
            ...
            # Create sparkle effect (smaller, more subtle)
            particles = []
        ...
        for effect in self.effects:
            ...
            if "particles" in effect:
                for particle in effect["particles"]:
                    ...
                    img = self.particle_images[particle["color_idx"] % len(self.particle_images)]
                    ...
                    scaled_img = pygame.transform.scale(img, (particle_size, particle_size))
                    ...
                    surface.blit(
                        scaled_img, 
                        (int(screen_x - particle_size/2), int(screen_y - particle_size/2))
                    )
...
class UIStateRenderer:
        ...
        for trans_type, trans in list(self.transitions.items()):
            if trans["active"]:
                ...
                if trans_type == "fade_in":
                    self.fade_alpha = 255 - int(255 * (trans["progress"] / trans["duration"]))
    ...
    def render(self, surface: pygame.Surface) -> None:
        ...
        draw_text(
            surface,
            f"Player angle: {self.player.angle}",
            10, 70,
            16, (255, 255, 255)
        )
        draw_text(
