...
def main():
    ...
    ship_panel.update_system_status("engines", {
        "status": "online",
        "efficiency": 95,
        "power_usage": 15
    })
    
    ship_panel.update_system_status("weapons", {
        "status": "online",
        "efficiency": 90,
        "power_usage": 20
    })
    
    ship_panel.update_system_status("shields", {
        "status": "online",
        "efficiency": 85,
        "power_usage": 25
    })
    
    ship_panel.update_system_status("sensors", {
        "status": "online",
        "efficiency": 100,
        "power_usage": 5
    })
    
    ship_panel.update_system_status("life_support", {
        "status": "online",
        "efficiency": 100,
        "power_usage": 10
    })
    ...
    while running:
            ...
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_ESCAPE:
                    ...
                    if random.random() < 0.3:
                        ...
                        ship_panel.update_system_status(damaged_system, {
                            "status": status,
        ...
        if current_time - last_update >= 0.1:  # Update 10 times per second for smoother animation
            ...
            if current_shield < max_shield:
                ...
                if can_recharge:
                    new_shield = min(max_shield, current_shield + recharge_rate * delta_time)
                    ship_panel.update_hull_shield(
            ...
            power_usage = sum(system["power_usage"] for system in ship_panel.systems.values())
            ...
            if system_damage_timer >= 5.0:
                ...
                # Update the system
                ship_panel.update_system_status(system_to_update, {
                    "status": status,
                    "efficiency": new_efficiency
                })
