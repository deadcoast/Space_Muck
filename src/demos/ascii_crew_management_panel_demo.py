...
class ASCIICrewManagementPanelDemo:
        ...
        for font_name in fonts_to_try:
            try:
                ...
                with contextlib.suppress(Exception):
                    ...
                    if (
                        test_surface.get_width() >= len(test_chars) * 7
    ...
    def _recruit_initial_crew(self) -> None:
        ...
        logging.info(
            f"Recruited initial crew: {len(self.crew_system.get_all_crew_members())} members"
        )
    ...
    def _on_unassign_crew(self, crew_id: str) -> None:
        ...
        if result["success"]:
            self._add_message(
                f"Unassigned {result['crew_member'].name} from {result['station']}"
            )
        else:
    ...
    def _on_train_crew(self, crew_id: str, skill_to_train: str) -> None:
        ...
        if result["success"]:
            self._add_message(
                f"{crew_member.name} trained {selected_skill} (+{result['skill_increase']})"
            )
        else:
    ...
    def _process_level_ups(self, level_ups):
        ...
        for level_up in level_ups:
            self._add_message(
                f"{level_up['name']} leveled up to level {level_up['new_level']}!"
            )
