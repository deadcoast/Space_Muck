...
class MultiSpeciesPopulation:
    ...
    def __init__(
        ...
        if len(base_growth_rates) != species_count:
            raise ValueError(f"Growth rates list length ({len(base_growth_rates)}) must match species count ({species_count})")
        if len(carrying_caps) != species_count:
            raise ValueError(f"Carrying capacity list length ({len(carrying_caps)}) must match species count ({species_count})")
        ...
        if interaction_matrix is None:
            ...
            if len(interaction_matrix) != species_count:
                raise ValueError(f"Interaction matrix rows ({len(interaction_matrix)}) must match species count ({species_count})")
            for row in interaction_matrix:
                if len(row) != species_count:
                    raise ValueError(f"Interaction matrix columns must match species count ({species_count})")
    ...
    def multi_species_step(
        ...
        if len(populations) != self.species_count:
            raise ValueError(f"Population list length ({len(populations)}) must match species count ({self.species_count})")
        ...
        for i in range(self.species_count):
            ...
            for j in range(self.species_count):
                if i != j and populations[j] > 0:  # Skip self-interaction and extinct species
                    interaction_term += self.interaction_matrix[i][j] * populations[j] / self.carrying_capacities[i]
    ...
    def set_interaction(self, species1: int, species2: int, value: float) -> None:
        ...
        """
        if not (0 <= species1 < self.species_count and 0 <= species2 < self.species_count):
            raise ValueError(f"Species indices must be between 0 and {self.species_count-1}")
...
class DelayedGrowthPopulation:
    ...
    def __init__(
        ...
        self.history = collections.deque([0.0] * self.history_length, maxlen=self.history_length)
    ...
    def update_population(self, current_population: float, resources: float = 0.0) -> float:
        """
        ...
        delayed_effect = delayed_effect / sum(self.delay_multipliers[:self.delay_steps + 1])
    ...
    def set_delay_multipliers(self, multipliers: List[float]) -> None:
        ...
        if len(multipliers) != self.delay_steps + 1:
            raise ValueError(f"Multiplier list length ({len(multipliers)}) must match delay_steps + 1 ({self.delay_steps + 1})")
...
class StageStructuredPopulation:
    ...
    def get_stage_distribution(self, stage_populations: Dict[str, float]) -> Dict[str, float]:
        """
