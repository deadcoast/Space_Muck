...
class MixedIntegerMineralOptimizer:
    ...
    def optimize_with_integer_constraints(
        ...
        feed_vars = self._create_feed_variables(
            total_minerals, binary_decisions, integer_minerals
        )
        ...
        self._add_conservation_constraints(
            prob, feed_vars, sell_vars, total_minerals, binary_decisions
        )

        # Add minimum feeding constraints
        self._add_min_feeding_constraints(
            prob, feed_vars, min_feeding_requirements, total_minerals, binary_decisions
        )
        ...
        self._add_exclusive_group_constraints(
            prob, feed_vars, exclusive_groups, binary_decisions, total_minerals
        )
    ...
    def _create_feed_variables(
        self, total_minerals, binary_decisions, integer_minerals
    ):
        ...
        for m in self.mineral_types:
            if binary_decisions and m in binary_decisions:
                ...
                # Integer variables
                feed_vars[m] = pl.LpVariable(
                    f"Feed_{m}", 0, total_minerals.get(m, 0), pl.LpInteger
                )
            else:
                # Continuous variables
                feed_vars[m] = pl.LpVariable(
                    f"Feed_{m}", 0, total_minerals.get(m, 0), pl.LpContinuous
                )
        return feed_vars
    ...
    def _create_sell_variables(self, total_minerals, binary_decisions):
        ...
        return {
            m: pl.LpVariable(f"Sell_{m}", 0, total_minerals.get(m, 0), pl.LpContinuous)
            for m in self.mineral_types
            if not binary_decisions or m not in binary_decisions
        }

    def _build_objective_terms(
        self, feed_vars, sell_vars, total_minerals, feed_benefits, sell_values
    ):
        """Helper method to build objective function terms"""
        profit_term = pl.lpSum(
            [
                sell_vars.get(m, total_minerals.get(m, 0) - feed_vars[m])
                * sell_values.get(m, 0)
                for m in self.mineral_types
            ]
        )
        appeasement_term = pl.lpSum(
            [feed_vars[m] * feed_benefits.get(m, 0) for m in self.mineral_types]
        )
    ...
    def _set_objective_function(
        ...
        if aggression_coef <= 0:
            ...
            prob += (
                1 - aggression_coef
            ) * profit_term + aggression_coef * appeasement_term

    def _add_conservation_constraints(
        self, prob, feed_vars, sell_vars, total_minerals, binary_decisions
    ):  # noqa: ARG001 # NOSONAR
        ...
        for m in self.mineral_types:
            if binary_decisions and m in binary_decisions:
                ...
                prob += sell_vars.get(
                    m,
                    total_minerals.get(m, 0) - feed_vars[m] * total_minerals.get(m, 0),
                ) <= total_minerals.get(m, 0)
            else:
                # Standard conservation constraint
                prob += feed_vars[m] + sell_vars.get(
            ...
            for m, min_amount in min_feeding_requirements.items():
                if m in feed_vars:
                    if binary_decisions and m in binary_decisions:
                        ...
                        prob += (
                            feed_vars[m] * total_minerals.get(m, 0)
                            >= min_amount * feed_vars[m]
                        )
                    else:
    ...
    def _add_exclusive_group_constraints(
        self, prob, feed_vars, exclusive_groups, binary_decisions, total_minerals
    ):  # noqa: ARG001 # NOSONAR
        ...
        for group_idx, group in enumerate(exclusive_groups):
            self._add_single_exclusive_group(
                prob, feed_vars, group, group_idx, binary_decisions, total_minerals
            )

    def _add_single_exclusive_group(
        self, prob, feed_vars, group, group_idx, binary_decisions, total_minerals
    ):  # noqa: ARG001 # NOSONAR
        ...
        for m in group:
            ...
            if binary_decisions and m in binary_decisions:
                ...
                group_vars[m] = pl.LpVariable(
                    f"Use_{m}_Group{group_idx}", 0, 1, pl.LpBinary
                )
    ...
    def _extract_results(self, feed_vars, total_minerals, binary_decisions):
        ...
        for m in self.mineral_types:
            if binary_decisions and m in binary_decisions:
                ...
                feed_result[m] = (
                    total_minerals.get(m, 0) * feed_vars[m].value()
                    if feed_vars[m].value() > 0.5
                    else 0
                )
            else:
                feed_result[m] = (
                    feed_vars[m].value() if feed_vars[m].value() is not None else 0
                )
        return feed_result
