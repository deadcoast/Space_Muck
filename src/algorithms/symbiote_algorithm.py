...
class SymbioteEvolutionAlgorithm:
    ...
    def apply_environmental_effects(self, grid, mineral_map, hostility):
        ...
        # More minerals = more favorable environment
        survival_mask = grid & (np.random.random(grid.shape) < (1 - hostility + mineral_map * 0.5))
        ...
        if competition_factor > 0.7:
            ...
            for i in range(1, num_colonies + 1):
                ...
                if size_ratio < 0.3:
                    ...
                    death_mask = (labeled_grid == i) & (np.random.random(grid.shape) < 0.3)
                    grid[death_mask] = 0
        else:
            # Cooperation: colonies may bridge together
            centers = ndimage.center_of_mass(grid, labeled_grid, range(1, num_colonies + 1))
            ...
            for i in range(num_colonies):
                for j in range(i+1, num_colonies):
                    ...
                    # If colonies are relatively close, try to bridge them
                    if distance < 20:
                        # Simple bridge-building logic
                        pass
    ...
    def get_colony_stats(self, grid, labeled_grid, num_colonies):
        ...
        centroids = ndimage.center_of_mass(grid, labeled_grid, range(1, num_colonies + 1))
