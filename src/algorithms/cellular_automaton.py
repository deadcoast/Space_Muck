...
def diffuse_energy(energy_grid: np.ndarray, decay_rate: float = 0.02, spread_rate: float = 0.1) -> np.ndarray:
    """
    ...
    kernel = np.array([[1, 1, 1], [1, 0, 1], [1, 1, 1]])
    neighborhood = signal.convolve2d(
        energy_grid, kernel, mode="same", boundary="wrap"
    )
