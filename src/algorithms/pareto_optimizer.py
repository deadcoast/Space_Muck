"""
src/algorithms/pareto_optimizer.py
...
class MultiObjectiveOptimizer:
    ...
    def _calculate_appeasement_bounds(
        ...
        max_appease_solution = {
            m: total_minerals.get(m, 0.0) for m in self.mineral_types
        }
        max_appeasement = sum(
            max_appease_solution[m] * feed_benefits.get(m, 0.0)
        ...
        min_appeasement = sum(
            min_appease_solution[m] * feed_benefits.get(m, 0.0)
            for m in self.mineral_types
        )
    ...
    def _setup_optimization_problem(
        ...
        def objective(x, mineral_types=mineral_order, num_minerals=n):
            ...
            selling = {
                m: total_minerals.get(m, 0.0) - feeding[m] for m in mineral_types
            }
        ...
        def appeasement_constraint(
            x,
            mineral_types=mineral_order,
            num_minerals=n,
            target_level=appeasement_level,
        ):
            # Constraint: appeasement >= appeasement_level
            feeding = {mineral_types[i]: x[i] for i in range(num_minerals)}
            actual_appeasement = sum(
                feeding[m] * feed_benefits.get(m, 0.0) for m in mineral_types
            )
        ...
        if min_feeding_requirements:
            for i, m in enumerate(mineral_order):
                if m in min_feeding_requirements:
                    ...
                    min_feeding_constraint = create_min_feeding_constraint(
                        i, min_feeding_requirements[m]
                    )
    ...
    def _extract_solution(
        ...
        appeasement = sum(
            solution[m] * feed_benefits.get(m, 0.0) for m in mineral_order
        )

        return ParetoOptimalSolution(
            solution=solution,
            objective_values={"profit": profit, "appeasement": appeasement},
        )
    ...
    def epsilon_constraint_method(
        ...
        for appeasement_level in appeasement_levels:
            ...
            mineral_order, bounds, constraints, x0, objective = (
    ...
    def weighted_sum_preference(
        ...
        def preference(objective_values: Dict[str, float]) -> float:
            return profit_weight * objective_values.get(
                "profit", 0.0
