...
from src.algorithms.economy_decision import MineralResourceOptimizer, compute_multi_faction_payoff
...
class SpatialMineralOptimizer(MineralResourceOptimizer):
    ...
    def __init__(
        ...
        self.pde_system = AdvectionDiffusionSystem(
            ...
            diffusion_coeffs={m_type: diffusion_coeffs.get(m_type, 0.1) for m_type in mineral_types},
            dt=dt,
    ...
    def add_mineral_source(self, mineral_type: str, position: Tuple[int, int], 
    ...
    def compute_spatial_payoff_matrix(
        ...
        if n_factions > 1:
            self._optimize_resource_allocation(n_factions, faction_positions, influence_radius)
        ...
        for i, position in enumerate(faction_positions):
            ...
            resource_values = np.array([local_resources.get(m, 0.0) for m in self.mineral_types])
            if resource_values.sum() > 0:
                resource_multiplier = 1.0 + 0.5 * (resource_values / resource_values.max()).mean()
            else:
    ...
    def _optimize_resource_allocation(self, n_factions: int, faction_positions: List[Tuple[int, int]], influence_radius: int):
        """
        ...
        for m_type in self.mineral_types:
            for i in range(n_factions):
                ...
                field = self.pde_system.fields[m_type]
                local_value = float(np.mean(field[max(0, x-influence_radius):min(self.width, x+influence_radius+1), 
                                            max(0, y-influence_radius):min(self.height, y+influence_radius+1)]))
        ...
        if result.success:
            return {
                m_type: {
                    f: result.x[m_idx * n_factions + f] for f in range(n_factions)
                }
                for m_idx, m_type in enumerate(self.mineral_types)
            }
        return None
