...
from src.world.procedural_generation import create_field_with_multiple_algorithms
...
from src.ui.draw_utils import draw_text, draw_panel, draw_minimap, draw_progress_bar, draw_button
from src.utils.logging_setup import setup_logging, log_exception, LogContext, log_performance_start, log_performance_end, log_memory_usage
import gc
import logging
import os
import sys
...
class Game:
    ...
    def initialize_visual_settings(self) -> None:
        ...
        try:
            ...
            pygame.draw.circle(self.cursor_surface, COLOR_PLAYER, 
                               (self.cursor_size//2, self.cursor_size//2), self.cursor_size//2)
            pygame.draw.circle(self.cursor_surface, (255, 255, 255), 
                               (self.cursor_size//2, self.cursor_size//2), self.cursor_size//2, 1)
    ...
    def add_welcome_notifications(self) -> None:
        ...
        self.notifier.add("Use ARROW KEYS to move your mining ship", category="system", duration=240)
        self.notifier.add("Press SPACE to mine asteroids", category="system", duration=240)
        self.notifier.add("Press S to open the shop", category="system", duration=240)
        self.notifier.add("Press N to toggle notification panel", category="system", duration=240)
        ...
        self.notifier.add("Press A to toggle auto-mining", category="system", duration=240)
        self.notifier.add("Press H to display control help", category="system", duration=240)
    ...
    def regenerate_field(self) -> None:
        ...
        # Notify player
        self.notifier.add(f"Field regenerated with seed {self.seed}!", category="event", importance=2)
    ...
    def handle_events(self) -> None:
        ...
        for event in pygame.event.get():
            ...
            if self.state == STATE_SHOP and self.shop.handle_event(event, self.player, self.field):
                continue
    ...
    def handle_key_press(self, key: int) -> None:
        ...
        if key == pygame.K_ESCAPE:
            ...
            # This is handled by the notification manager
            pass
        ...
        # State-specific keys
            ...
            # Most shop keys are handled by the shop class
            pass
    ...
    def handle_play_state_keys(self, key: int) -> None:
        ...
        if key == pygame.K_s:
            ...
            if self.player.add_ship():
                self.notifier.add("New mining ship added to your fleet!", category="mining")
            else:
                self.notifier.add("Cannot add more ships (max reached or insufficient funds)", category="mining")
    ...
    def mine(self) -> None:
        ...
        with LogContext("Mining operation"):
            ...
            if minerals_mined > 0:
                self.notifier.add(
                    f"Mined {minerals_mined} minerals",
                    category="mining"
                )
    ...
    def feed_symbiotes(self) -> None:
        ...
        if results["fed"] > 0:
            self.notifier.add(
                f"Fed symbiotes with {results['fed']} minerals",
                category="race"
            )
        else:
    ...
    def handle_auto_upgrade(self) -> None:
        ...
        if not self.auto_upgrade or self.frame_counter % self.auto_upgrade_interval != 0:
            return
        ...
        if best_upgrade:
            self.shop.purchase_upgrade(best_upgrade, self.player, self.field, self.notifier)
    ...
    def update_play_state(self) -> None:
        ...
        if self.auto_mine and self.frame_counter % 30 == 0:  # Every half second at 60 FPS
            minerals_mined = self.player.mine(self.field)
            if minerals_mined > 0:
                self.stats["total_mined"] += minerals_mined
        ...
        if current_time - self.last_field_update_time > self.update_interval / 60:
            ...
            for race_id, income in race_incomes.items():
                if income > 500:
                    race = next((r for r in self.field.races if r.race_id == race_id), None)
                    if race:
                        self.notifier.add(
        ...
        for race in self.field.races:
            if (race.evolution_points >= race.evolution_threshold and 
                self.frame_counter % 300 == race.race_id % 300):  # Stagger evolutions
                ...
                if metrics and metrics["center"]:
                    logging.info(f"Race {race.race_id} territory: " + 
        ...
        if self.frame_counter % 120 == 0:  # Every 2 seconds at 60 FPS
            ...
            for dy in range(-3, 4):
                for dx in range(-3, 4):
                    ...
                    if (0 <= nx < self.field.width and 0 <= ny < self.field.height and
                        self.field.grid[ny, nx] > 0 and self.field.rare_grid[ny, nx] == 3):  # Anomaly
...
class Game:
    ...
    def __init__(self) -> None:
        ...
        self.notifier.add("Welcome to Space Muck! Press H for help.", duration=600, category="system", importance=3)
        self.notifier.add("Use WASD or arrow keys to move.", duration=600, category="system")
    ...
    def initialize_symbiote_races(self) -> None:
        ...
        with LogContext("Race Population Generation"):
            ...
            logging.info(f"Starting with race {starting_race.race_id} ({starting_race.trait})")
    ...
    def handle_filter_keys(self, key: int) -> None:
        ...
        if key in self.filter_shortcuts:
            ...
            if filter_name == "cycle_detail":
                ...
                self.notifier.add(f"Detail level: {self.visual_filters['detail_level']}", category="system")
            else:
                ...
                self.notifier.add(f"{filter_name.replace('_', ' ').title()}: {status}", category="system")
    ...
    def auto_upgrade_logic(self) -> None:
        ...
        # Get the list of affordable upgrades
        affordable_upgrades = []
        for option in self.shop.options:
            if (option.get("category", "") == "ship" and 
                    self.player.currency >= option.get("cost", float('inf')) and
                    not option.get("locked", False) and
                    (option.get("max_level", None) is None or 
                     option.get("current_level", 0) < option.get("max_level", 0))):
        ...
        if affordable_upgrades:
            ...
            success = self.shop.purchase_upgrade(
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if event.key == pygame.K_F1:
                    ...
                    self.notifier.add(f"Screenshot saved: {filename}", category="system")
                ...
                if self.state == STATE_PLAY:
                    ...
                    if event.key == pygame.K_ESCAPE:
                        self.state = STATE_PLAY
                    elif self.shop.handle_event(event, self.player, self.field, self.notifier):
                        self.state = STATE_PLAY
            ...
            # Handle mouse events
                ...
                if self.show_minimap:
                    minimap_rect = pygame.Rect(
                        10, WINDOW_HEIGHT - MINIMAP_SIZE - 10, 
    ...
    def handle_play_state_input(self, event: pygame.event.Event) -> None:
        ...
        if event.key == pygame.K_s and not pygame.key.get_mods() & pygame.KMOD_SHIFT:
            ...
            if reward > 0:
                self.notifier.add(f"Mined asteroids: +{reward} credits", category="mining")
                # Add visual effect
                if self.visual_filters["show_animations"]:
                    self.effects_renderer.add_effect(
                        "sparkle", self.player.x, self.player.y, 
            ...
            if self.visual_filters["show_animations"]:
                self.effects_renderer.add_effect(
                    "bloom", self.player.x, self.player.y, 
                    size=15, color=COLOR_ASTEROID_RARE
                )
        ...
        # Toggle auto-upgrade
            ...
            if hasattr(self.player, "auto_upgrade"):
                ...
                self.notifier.add("Auto-upgrades enabled", category="upgrade", duration=120)
                
        # Symbiote feeding actions
            ...
            if minerals_fed > 0:
                ...
                if self.visual_filters["show_animations"]:
                    self.effects_renderer.add_effect(
                        "spray", self.player.x, self.player.y, 
                        size=10, color=COLOR_RACE_1
                    )
            else:
                self.notifier.add(
                    "Not enough minerals for feeding", 
                    category="race", 
                    importance=1
                )
        ...
        # Ship building        
            ...
            if self.player.currency >= self.player.ship_cost and \
               self.player.mining_ships < self.player.max_mining_ships:
                success = self.player.add_ship()
                if success:
                    self.notifier.add(
                    ...
                    if self.visual_filters["show_animations"]:
                        self.effects_renderer.add_effect(
                            "explosion", self.player.x, self.player.y, 
                            size=5, color=COLOR_PLAYER
                        )
    ...
    def update(self) -> None:
        ...
        if self.frame_counter % self.update_interval == 0:
            ...
            if fleet_results["ships_lost"] > 0:
                ...
                if self.visual_filters["show_animations"] and fleet_results["ships_lost"] > 0:
            ...
            for race in self.field.races:
                ...
                if self.frame_counter % 300 == 0:
                    hunger_status = (
                        "Starving" if race.hunger > 0.8 else
                        "Hungry" if race.hunger > 0.4 else
                        "Satiated"
                    )
    ...
    def draw(self) -> None:
        ...
        if SHOW_FPS:
            draw_text(
                ...
                10,
                size=16,
                color=COLOR_TEXT if self.clock.get_fps() >= FPS*0.9 else COLOR_WARNING
            )
    ...
    def draw_minimap(self, surface: pygame.Surface) -> None:
        ...
        minimap_rect = pygame.Rect(
            padding, 
            WINDOW_HEIGHT - minimap_size - padding, 
            minimap_size, 
            minimap_size
        )
