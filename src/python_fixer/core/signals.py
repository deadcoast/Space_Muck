...
class SignalManager:
    ...
    def register_signal(self, sig: int) -> None:
        ...
        if sig not in self._active_signals:
            ...
            self.logger.debug(f"Installed handler for {signal.Signals(sig).name if hasattr(signal, 'Signals') else f'signal {sig}'}")
    ...
    def unregister_signal(self, sig: int) -> None:
        ...
        if sig in self._active_signals:
            ...
            self.logger.debug(f"Restored original handler for {signal.Signals(sig).name if hasattr(signal, 'Signals') else f'signal {sig}'}")
