...
def test_signature_visitor_without_libcst(temp_py_file, monkeypatch):
    ...
    simple_func = next(
        sig for sig in visitor.signatures if sig.name == "simple_function"
    )
    ...
    untyped_func = next(
        sig for sig in visitor.signatures if sig.name == "untyped_function"
    )
    ...
    typed_method = next(
        sig for sig in visitor.signatures if sig.name == "method_with_types"
    )
...
def test_signature_visitor_class_parsing(temp_py_file, monkeypatch):
    ...
    untyped_method = next(
        sig for sig in test_class_methods if sig.name == "method_without_types"
    )
...
'''
    ...
    multiline_func = next(
        sig for sig in visitor.signatures if sig.name == "func_with_multiline_doc"
    )
    ...
    single_line_func = next(
        sig for sig in visitor.signatures if sig.name == "func_with_single_line_doc"
    )
    assert single_line_func.docstring.strip() == "Single line docstring."

    # Test method docstring with indentation
    method = next(sig for sig in visitor.signatures if sig.name == "method_with_doc")
...
@pytest.mark.torch
def test_type_inference_with_torch(
    temp_py_file, mock_type_inference_model, monkeypatch
):
...
def test_signature_component_creation():
    ...
    component = SignatureComponent(
        name="test_param",
        type_info=TypeInfo(type_hint="int", inferred_type=None, confidence=1.0),
        is_optional=True,
    )
...
def test_code_signature_creation():
    ...
    components = [
        SignatureComponent(
            name="param1",
            type_info=TypeInfo(type_hint="str", inferred_type=None, confidence=1.0),
        )
    ]
    signature = CodeSignature(
        ...
        return_type=TypeInfo(type_hint="bool", inferred_type=None, confidence=1.0),
    )
...
"""
    ...
    visitor = SignatureVisitor(
        file_path, type_inference_model=FailingModel(), enable_type_inference=True
    )
    visitor.analyze()
