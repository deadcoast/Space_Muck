...
def invalid_implementation():
    ...
    class InvalidImpl:
        ...
        def method_b(self, y: float) -> List[str]:  # Missing Optional, wrong return type
            return ["invalid"]
...
def test_optional_type_validation():
    ...
    assert not result.is_valid
    assert "expected Union[<class 'int'>, <class 'NoneType'>], got str" in result.errors[0]

# Protocol implementation tests
@pytest.mark.unit
def test_protocol_implementation(sample_protocol_class, valid_implementation, invalid_implementation):
    ...
    assert any("method_a" in msg and "expected <class 'int'>" in msg for msg in error_messages), (
        "Missing type mismatch error for method_a parameter type"
    )
    assert any("method_b" in msg and "expected Union[<class 'float'>, <class 'NoneType'>]" in msg for msg in error_messages), (
