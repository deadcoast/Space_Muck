...
def test_valid_import_validation(invalid_import_structure: Path) -> None:
    ...
    assert analyzer.validate_all_imports(), "Valid imports should be validated successfully"
    assert len(analyzer.get_invalid_imports()) == 0, "There should be no invalid imports"
...
def test_invalid_relative_import_validation(invalid_import_structure: Path) -> None:
    ...
    assert any("nonexistent_module" in msg for msg in error_messages), "Error for nonexistent_module not found"
    assert any("parent_pkg" in msg for msg in error_messages), "Error for parent_pkg not found"
...
def test_invalid_absolute_import_validation(invalid_import_structure: Path) -> None:
    ...
    # Check error messages
    error_messages = analyzer.get_import_errors()
    assert any("nonexistent_package" in msg for msg in error_messages), "Error for nonexistent_package not found"
    assert any("another_nonexistent_package" in msg for msg in error_messages), "Error for another_nonexistent_package not found"
...
""")
    ...
    assert analyzer._package_path == "invalid_pkg.nested.pkg", "Package path should be correctly resolved"
    ...
    assert any("invalid_pkg" in msg for msg in error_messages), "Error for invalid_pkg not found"
...
def test_direct_import_info_validation() -> None:
    ...
    assert not abs_import.validate(), "Nonexistent absolute import should fail validation"
    assert abs_import.error_message is not None, "Error message should be set"
    assert "Cannot resolve absolute import" in abs_import.error_message, "Error message should mention absolute import"
    
    # Test relative import validation without package path
    rel_import = ImportInfo(module="module", is_relative=True, level=1)
    assert not rel_import.validate(), "Relative import without package path should fail validation"
    assert rel_import.error_message is not None, "Error message should be set"
    assert "Cannot resolve relative import without package path" in rel_import.error_message, "Error message should mention missing package path"
    
    # Test relative import validation with invalid level
    rel_import2 = ImportInfo(module="module", is_relative=True, level=3)
    assert not rel_import2.validate("pkg.subpkg"), "Relative import with excessive level should fail validation"
    assert rel_import2.error_message is not None, "Error message should be set"
    assert "Relative import level" in rel_import2.error_message, "Error message should mention level issue"
