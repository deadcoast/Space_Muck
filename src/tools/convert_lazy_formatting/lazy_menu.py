...
try:
    ...
    module_path = os.path.join(
        os.path.dirname(__file__), "convert_to_lazy_formatting.py"
    )
    spec = importlib.util.spec_from_file_location(
        "convert_to_lazy_formatting", module_path
    )
...
class InteractiveMenu:
    ...
    def __init__(self):
        ...
        self.options = {"dry_run": False, "verbose": False, "skip_complex": False}
    ...
    def draw_header(self):
        ...
        print("=" * self.width)
        print(
            "Convert eager logging calls (f-strings, .format) into lazy-format style."
        )
        print("-" * self.width)
    ...
    def remove_file(self):
        ...
        try:
            if not (
                index := int(
                    input(
                        f"Enter file number to remove (1-{len(self.files_to_process)}): "
                    )
                )
            ):
    ...
    def toggle_option(self, option: str):
        ...
        if option in self.options:
            ...
            self.last_message = (
                f"Option '{option}' is now {'ON' if self.options[option] else 'OFF'}."
            )
        else:
